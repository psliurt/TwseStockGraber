...
{
    /// <summary>
    /// 交易資訊->盤後資訊->每日收盤行情  ETF ETN 跟受益證券 選項 這個類別會由 MiIndexGraber取代
    /// </summary>
    public class MiIndexEtfGraber
    {

        public void DoJob(DateTime dataDate)
        {
            List<string> _typeList = new List<string>();
            _typeList.Add("0099P"); //ETF
            _typeList.Add("029999"); //ETN
            _typeList.Add("019919T"); //受益證券
            ...
            foreach (string type in _typeList)
            {
                ...
                MI_INDEX_ETF_Rsp rsp = JsonConvert.DeserializeObject<MI_INDEX_ETF_Rsp>(responseContent);
        ...
        private void SaveToDatabase(MI_INDEX_ETF_Rsp rsp, string type, DateTime dataDate)
        {
