...
namespace TwStockGrabBLL.Logic.DeskGraber
{
    ...
    /// </summary>
    public class D3itrdsumYearlyGraber : DGraber
    {

        public D3itrdsumYearlyGraber() : base()
        {
            this._graberClassName = typeof(D3itrdsumYearlyGraber).Name;
        ...
        {
            List<d_3itrdsum_yearly> tmpAddList = new List<d_3itrdsum_yearly>();
            List<d_3itrdsum_yearly> tmpUpdateList = new List<d_3itrdsum_yearly>();
            List<d_3itrdsum_yearly> tmpDataList = null;
            using (TwStockDataContext context = new TwStockDataContext())
            {
                tmpDataList = context.Set<d_3itrdsum_yearly>().Where(x => x.data_date == dataDate).ToList();
            }
            ...
            foreach (var data in rsp.aaData)
            {
                ...
                d_3itrdsum_yearly existItem = tmpDataList.Where(x => x.item_name == itemName && x.data_date == dataDate).FirstOrDefault();
                if (existItem == null)
                {
                    tmpAddList.Add(new d_3itrdsum_yearly
                    {
                        data_date = dataDate,
                        item_name = itemName,
            ...
            using (TwStockDataContext context = new TwStockDataContext())
            {
                context.d_3itrdsum_yearly.AddRange(tmpAddList);
                foreach (var item in tmpUpdateList)
                {
                    context.Entry<d_3itrdsum_yearly>(item).State = System.Data.Entity.EntityState.Modified;
                }
                context.SaveChanges();
            }
        }
        ...
        private string GetWebContent(DateTime date)
        {
            ...
            ///https://www.tpex.org.tw/web/stock/3insti/3insti_summary/3itrdsum_result.php?l=zh-tw&t=Y&p=1&d=2019&_=1578804243965
            string url = string.Format("https://www.tpex.org.tw/web/stock/3insti/3insti_summary/3itrdsum_result.php?l={0}&t={1}&p={2}&d={3}&_={4}",
                lang, dataType, stockSelectType, adYear, paramUnderLine);
